...
def store_target_report(callback_context: CallbackContext):
    ...
    try:
        project_id = callback_context.state.get('project_id')
        project_id = project_id.replace('"','')
        sales_proc = callback_context.state.get('sales_agent_input')
        try:
            # Try to parse the string as JSON
            data = json.loads(sales_proc)
...
# conditional_target_html_composer = LlmAgent(
#     model=config.worker_model,
...
#         Check the sales_agent_input: {sales_agent_input}    
#         If sales_agent_input contains "skip_sales": true, then output exactly:
#         {{"skipped": true, "reason": "No specific target organizations identified in user input"}}. Otherwise follow the html template,"""+TARGET_TEMPLATE,
...
def harbinger_message(callback_context:CallbackContext):
    ...
    requests.put(f"http://localhost:4005/project/project-status-update/{project_id}/",
...
simplified_intelligence_agent = SequentialAgent(
    ...
    sub_agents=[
        ...
        # conditional_target_html_composer
    ]
)
