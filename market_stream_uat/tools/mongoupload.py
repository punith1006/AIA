...
def update_project_report(project_id: str, report: str, report_type: str, html_report: str = ""):
    ...
    if html_report !="":
        ...
        format = "sub_status"
    requests.put(f"http://localhost:4005/project/project-status-update/{project_id}/",headers = {'Content-Type': 'application/json'}, data = json.dumps({format: f"{report_type} updated",}), timeout=10)
