...
class ApolloSearchParameters(BaseModel):
    ...
    person_seniorities: list[str] = Field(default=['senior'], description="Seniority levels")
...
persona_intelligence_generator = LlmAgent(
    ...
    **Person-Level Filters:**
    ...
    - person_seniorities: select the seniorities tags based on relevance with person_titles, only choose the seniorty tags from the tags presented within the following brackets (owner, founder, c_suite, partner, vp, head, director, manager, senior, entry, intern)
...
persona_quality_validator = LlmAgent(
    ...
    **2. Persona Completeness:**
    ...
    - seniority coverage, ensure the values are chosen are only from the values present within these brackets (owner, founder, c_suite, partner, vp, head, director, manager, senior, entry, intern)
...
prospect_researcher = SequentialAgent(
    ...
    sub_agents=[
        ...
        LoopAgent(
            ...
            max_iterations=3,  # Increased iterations for quality assurance
            sub_agents=[
