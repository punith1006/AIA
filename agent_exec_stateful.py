...
# Import VertexAiSessionService for sessions visible in ADK Web UI
...
# Configuration for Vertex AI Agent Engine (required for ADK Web UI visibility)
PROJECT_ID = os.getenv("GOOGLE_CLOUD_PROJECT", "your-gcp-project-id")
LOCATION = os.getenv("GOOGLE_CLOUD_LOCATION", "us-central1") 
AGENT_ENGINE_ID = os.getenv("GOOGLE_CLOUD_AGENT_ENGINE_ID", "your-agent-engine-id")

# Use VertexAiSessionService for sessions visible in ADK Web UI
session_service = VertexAiSessionService(PROJECT_ID, LOCATION)
APP_NAME = AGENT_ENGINE_ID  # Use Agent Engine ID as app_name
...
async def run_live_agent_ws(websocket: WebSocket):
    ...
    # Generate user ID (you might want to get this from authentication)
    user_id = f"user_{uuid.uuid4().hex}"
    
    # Check for existing sessions for this user or create a new one
    try:
        ...
        if existing_sessions and len(existing_sessions) > 0:
            ...
            logger.info(f"Created new persistent session: {session}")
...
@app.get("/persistent-sessions/{user_id}")
async def get_persistent_sessions(user_id: str):
    """Get all persistent sessions for a user from Vertex AI Agent Engine"""
    try:
        persistent_sessions = session_service.list_sessions(
            app_name=APP_NAME,
            user_id=user_id
        )
        return {
            "user_id": user_id,
            "backend": "vertex_ai_agent_engine",
            "web_ui_access": f"adk web --session_db_url=agentengine://{AGENT_ENGINE_ID}",
            "sessions": [
                {
        ...
        logger.error(f"Error fetching persistent sessions from Vertex AI: {e}")
